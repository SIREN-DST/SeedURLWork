...
for line in f1:
    ...
    try:
        ...
        continue
    suburlmapper[suburl]=seed
    seedmapper[seed]=dom

error=0

with open("Wiki-Seed-URL-Score","w") as f3:
    f3.write('')
import glob
for filename in glob.iglob('./Meta Data/*'):
    f2=open(filename,"r")
    for line in f2:
        temp=line.split(',')
        try:
        ...
        #print dom+','+seed+','+url+','+str(partscore)+'\n'
        
        try:
            with open("Wiki-Seed-URL-Score","a") as f3:
                f3.write(dom+','+seed+','+url+','+str(partscore)+'\n')
        except:
            continue
        count+=1
