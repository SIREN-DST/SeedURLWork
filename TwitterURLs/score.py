...
for category in level1:
    ...
    k=f.readline()
    while k!='':
        dic[k.strip()]=eval(f.readline())
        k=f.readline()
    f=open(category+"tweet",'r')
    for line in f:
        temp=line.split(',')
        url=temp[0].strip(' \n')
        tweet=temp[1].strip(' \n')
        ...
        for phrase in terms:
            ...
            flag=0
            for word in phrase.split():
                if word.lower() not in stopwords and word.lower() in tweet:
                    partscoretweet+=1.0/len(phrase)
                else:
                    flag=1
            if flag==0:
                fullscoretweet+=1
        print url+','+subdom+','+str(partscore)+','+str(fullscore)+','+str(partscoretweet)+','+str(fullscoretweet)+','+lastmod+','+domain+','+str(size)+','+alltext+','+str(indom)+','+str(outdom)+'\n'
        
        try:
            with open("TwitterURLScore"+category,"a") as f3:
                f3.write(url+','+subdom+','+tweet+','+str(partscore)+','+str(fullscore)+','+str(partscoretweet)+','+str(fullscoretweet)+','+lastmod+','+domain+','+str(size)+','+alltext+','+str(indom)+','+str(outdom)+'\n')
        except:
            continue
        count+=1
        print '##########',count
